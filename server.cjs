...
async function generateNexusCode(prompt, nexusId, classification) {
  ...
  const entityLabel = String(meta.primary_entity || 'Record').replace(/([a-z0-9])([A-Z])/g, '$1 $2').replace(/_/g, ' ').trim();
...
app.post('/api/generate', async (req, res) => {
  ...
  try {
    ...
    nexusData.primary_entity = String(nexusData.primary_entity || 'Record').replace(/([a-z0-9])([A-Z])/g, '$1 $2').replace(/_/g, ' ').trim();
